...
function R(l) {
  const t = (e) => me(e) ? toRaw(e) : Array.isArray(e) ? e.map((o) => t(o)) : isRef(e) || isReactive(e) || isProxy(e) ? t(toRaw(e)) : e;
  return t(l);
}
var he = { ref: "root" };
  ...
  setup(l, { expose: t, emit: e }) {
    ...
    _GET_ALL_GRID_OPTIONS().filter((r) => r != "gridOptions").forEach((r) => {
      watch(
        () => B[r],
        (s, v) => {
          (r === "rowData" && !w.value || r !== "rowData") && P(r, s), w.value = false;
        },
        { deep: true }
      );
    });
    const h = /* @__PURE__ */ new Set(["rowDataUpdated", "cellValueChanged", "rowValueChanged"]), m = useModel(l, "modelValue"), F = ref(false), w = ref(false), E = e;
    watch(
      m,
      (r, s) => {
        d.value && (w.value || (F.value = true, P("rowData", R(r), R(s))), w.value = false);
      },
      { deep: true }
    );
    const G = Ce(() => {
