              ...
              >
                <div class="mb-1.5 flex items-end justify-between gap-3">
                  <div>
                    ...
                    <p class="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wide text-slate-400">
                      <Icon name="fileText" class="h-3.5 w-3.5" /> {{ item.docs }} Dokumen
                    </p>
                  </div>
                  <span class="text-xs font-black text-[#00A3FF]">{{ item.days }} hari</span>
                </div>
          ...
          <article class="finance-glass-card finance-card-rose">
            ...
            <div class="finance-scrollpane max-h-[280px] space-y-3 overflow-y-auto pr-1">
              ...
              >
                ...
                <p class="min-w-0 flex-1 truncate text-left text-sm font-bold text-slate-700">{{ item.name }}</p>
                <span class="ml-auto text-xs font-black text-rose-600">{{ item.days }} hari</span>
              </div>
            </div>
          </article>
            ...
            <div class="finance-scrollpane finance-contract-list max-h-[280px] space-y-3 overflow-y-auto pr-1">
              <div v-for="item in belumBerkontrak" :key="item.id" class="finance-contract-item rounded-3xl border border-emerald-100 bg-emerald-50/45 p-4">
                <div class="mb-2 flex items-start justify-between gap-3">
                  ...
                  <span class="rounded-lg border border-emerald-100 bg-white/80 px-2 py-0.5 text-[10px] font-black text-emerald-600">{{ item.days }}h</span>
                </div>
                <div class="flex items-end justify-between">
                  <p class="text-[10px] font-black uppercase tracking-wide text-slate-400">Estimasi Nilai</p>
                  <p class="text-sm font-black text-emerald-700">Cost {{ item.cost }}</p>
                </div>
              </div>
            </div>
          </article>
