<template>
  <div class="card p-0 overflow-hidden">
    <div class="p-3 flex items-center gap-2 border-b border-base-300">
      <input v-model="q" class="input w-full md:w-80" placeholder="Search…" />
      <slot name="toolbar" />
    </div>
    <table class="w-full text-sm">
      <thead class="bg-base-200">
        <tr>
          <th v-for="h in headers" :key="h.key" class="text-left p-3">
            {{ h.label }}
          </th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="row in paged" :key="row.id" class="border-t border-base-300">
          <td v-for="h in headers" :key="h.key" class="p-3">
            <slot :name="`cell:${h.key}`" :row="row">{{ row[h.key] }}</slot>
          </td>
        </tr>
        <tr v-if="!paged.length">
          <td :colspan="headers.length" class="p-8 text-center opacity-70">
            No data.
          </td>
        </tr>
      </tbody>
